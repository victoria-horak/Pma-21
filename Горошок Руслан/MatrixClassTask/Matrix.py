            ...
            for iterator in range(len(self.matrix)):
                for jterator in range(len(second_matrix.get_row(iterator))):
                    for kterator in range(len(second_matrix.get_matrix())):
                        result_multi_matrix[iterator][jterator] += self.matrix[iterator][
                                                                       kterator] * second_matrix.get_item(kterator,
                                                                                                          jterator)
            return Matrix(result_multi_matrix)
        except IndexError:
            with open("sources\\resultMatrix.txt", "a+") as matrix_file:
                matrix_file.write("We cannot multiply this matrix")
            return Matrix([])
            ...
            for iterator in range(len(self.matrix)):
                for jterator in range(len(second_matrix.get_row(iterator))):
                    for kterator in range(len(second_matrix.get_matrix())):
                        try:
                            result_multi_matrix[iterator][jterator] += self.matrix[iterator][
                                                                           kterator] / second_matrix.get_item(
                                kterator, jterator)
                        except ZeroDivisionError:
                            with open("sources\\resultMatrix.txt", "a+") as matrix_file:
                                matrix_file.write("We cannot division by zero ")
                            return Matrix([])
            return Matrix(result_multi_matrix)
        except IndexError:
            with open("sources\\resultMatrix.txt", "a+") as matrix_file:
                matrix_file.write("We cannot division this matrix")
            return Matrix([])
